...
import {
  ...
  searchProductsForGuide,
  setGuideStatus,
  updateGuideContent,
  updateGuideSeo,
  uploadGuideImage,
} from "@/app/agency/guides-actions";
import type { AgencyGuideDetail } from "@/lib/guides/agency-queries";
...
export function GuideEditor({ guide, role }: { guide: AgencyGuideDetail; role: Role }) {
  ...
  return (
    <Card>
      <div className="flex items-center justify-between gap-4 border-b border-line px-2">
        <nav className="-mb-px flex gap-1 overflow-x-auto" aria-label="Guide sections">
          {tabs.map((t) => (
            <button
              key={t.id}
              type="button"
        ...
        <div className="pr-3">
          <PublishControl guideId={guide.id} status={guide.status} canPublish={canEditContent} />
        </div>
      </div>

      <div className="p-5 sm:p-6">
        {tab === "content" && <ContentForm guide={guide} readOnly={!canEditContent} />}
        {tab === "related" && <RelatedProductsPanel guide={guide} readOnly={!canEditContent} />}
        {tab === "seo" && <SeoForm guide={guide} readOnly={!canEditSeo} />}
      </div>
    </Card>
  );
}

function PublishControl({ guideId, status, canPublish }: { guideId: string; status: "draft" | "published"; canPublish: boolean }) {
    ...
    <button
      ...
      onClick={() =>
        startTransition(async () => {
          const result = await setGuideStatus({ guideId, status: target });
          if (!result.ok) {
...
function ContentForm({ guide, readOnly }: { guide: AgencyGuideDetail; readOnly: boolean }) {
  const [pending, startTransition] = useTransition();
  ...
  function handleSubmit(e: FormEvent<HTMLFormElement>) {
    ...
    startTransition(async () => {
      const result = await updateGuideContent({ guideId: guide.id, content: form.get("content"), categoryId: guide.category?.id ?? null });
      if (!result.ok) {
        toast.error(result.error);
        return;
      }
  ...
  return (
    <div className="space-y-8">
      <div className="space-y-2">
        ...
        {guide.featuredImagePath ? (
          <div className="relative w-48 overflow-hidden rounded-xl border border-line">
            ...
            <img src={guide.featuredImagePath} alt="" className="aspect-video w-full object-cover" />
            {!readOnly && (
              <button
                type="button"
                ...
                onClick={() =>
                  startUpload(async () => {
                    const result = await deleteGuideImage({ guideId: guide.id, storagePath: guide.featuredImagePath });
                    if (!result.ok) {
                      toast.error(result.error);
                      return;
                    }
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <fieldset disabled={readOnly || pending}>
          <div className="space-y-1.5">
            ...
            <textarea
              ...
              placeholder={"## Introduction\n\nWrite the guide here using Markdown — headings, **bold**, lists, and links are supported."}
            />
        ...
        {readOnly && <p className="text-sm text-ink-subtle">Your role doesn&apos;t have write access to guide content.</p>}
      </form>
    </div>
  );
}

function RelatedProductsPanel({ guide, readOnly }: { guide: AgencyGuideDetail; readOnly: boolean }) {
  const [pending, startTransition] = useTransition();
  ...
  function handleSearch(value: string) {
    ...
    startSearch(async () => {
      const result = await searchProductsForGuide(value);
      setResults(result.ok ? (result.data ?? []).filter((p) => !guide.relatedProducts.some((r) => r.id === p.id)) : []);
    });
  }
  ...
  return (
    <div className="space-y-5">
      <ul className="space-y-2">
          ...
          <li key={p.id} className="flex items-center justify-between rounded-lg border border-line px-3.5 py-2.5 text-sm">
            ...
            {!readOnly && (
              <button
                ...
                onClick={() =>
                  startTransition(async () => {
                    const result = await removeRelatedProduct({ guideId: guide.id, productId: p.id });
                    if (!result.ok) {
                      toast.error(result.error);
                      return;
                    }
        ...
        {guide.relatedProducts.length === 0 && <p className="text-sm text-ink-muted">No related products yet.</p>}
      </ul>
                ...
                results.map((p) => (
                  <li key={p.id}>
                    <button
                      ...
                      onClick={() =>
                        startTransition(async () => {
                          const result = await addRelatedProduct({ guideId: guide.id, productId: p.id });
                          if (!result.ok) {
                            toast.error(result.error);
                            return;
                          }
...
function SeoForm({ guide, readOnly }: { guide: AgencyGuideDetail; readOnly: boolean }) {
  const [pending, startTransition] = useTransition();
  ...
  function handleSubmit(e: FormEvent<HTMLFormElement>) {
    ...
    const form = new FormData(e.currentTarget);
    startTransition(async () => {
      const result = await updateGuideSeo({
        guideId: guide.id,
    ...
    <form onSubmit={handleSubmit} className="space-y-5">
      <fieldset disabled={readOnly || pending} className="space-y-5">
        <div className="space-y-1.5">
          ...
          <input id="title" name="title" defaultValue={guide.seo?.title ?? ""} maxLength={70} className={cx(inputStyles, "disabled:cursor-not-allowed disabled:opacity-60")} />
        </div>
        <div className="space-y-1.5">
          <label htmlFor="metaDescription" className="block text-sm font-medium text-ink">
            Meta description
          </label>
          <textarea id="metaDescription" name="metaDescription" defaultValue={guide.seo?.metaDescription ?? ""} maxLength={160} rows={2} className={cx(inputStyles, "resize-y disabled:cursor-not-allowed disabled:opacity-60")} />
        </div>
        <div className="space-y-1.5">
          <label htmlFor="ogTitle" className="block text-sm font-medium text-ink">
            Social share title
          </label>
          <input id="ogTitle" name="ogTitle" defaultValue={guide.seo?.ogTitle ?? ""} maxLength={70} className={cx(inputStyles, "disabled:cursor-not-allowed disabled:opacity-60")} />
        </div>
        <div className="space-y-1.5">
          <label htmlFor="ogDescription" className="block text-sm font-medium text-ink">
            Social share description
          </label>
          <textarea id="ogDescription" name="ogDescription" defaultValue={guide.seo?.ogDescription ?? ""} maxLength={160} rows={2} className={cx(inputStyles, "resize-y disabled:cursor-not-allowed disabled:opacity-60")} />
        </div>
        <div className="flex gap-6">
          <label className="flex items-center gap-2 text-sm text-ink">
            <input type="checkbox" name="robotsIndex" defaultChecked={guide.seo?.robotsIndex ?? true} />
            Allow search engines to index this page
          </label>
          <label className="flex items-center gap-2 text-sm text-ink">
            <input type="checkbox" name="robotsFollow" defaultChecked={guide.seo?.robotsFollow ?? true} />
