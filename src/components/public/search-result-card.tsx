...
export function SearchResultCard({ result }: { result: SearchResult }) {
  return (
    <li className="group overflow-hidden rounded-2xl border border-line bg-surface shadow-card transition hover:border-line-strong hover:shadow-pop">
      <Link href={result.url} className="block focus-visible:outline-none">
        <div className="aspect-[4/3] w-full overflow-hidden bg-sunken">
          {result.imagePath ? (
            ...
            <img src={result.imagePath} alt="" className="h-full w-full object-cover" />
          ) : (
            <PlaceholderImage className="h-full w-full" />
          )}
        </div>
        <div className="p-4">
          <span className={`inline-flex items-center rounded-full px-2 py-0.5 text-[11px] font-medium ${TYPE_BADGE_TONE[result.type]}`}>
          ...
          <h3 className="mt-2 font-display text-base leading-snug text-ink group-hover:text-brand-700">{result.title}</h3>
