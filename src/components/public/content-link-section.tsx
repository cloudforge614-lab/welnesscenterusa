  ...
  return (
    <section className="mt-16 border-t border-line pt-10" aria-labelledby={id}>
      <div className="flex items-end justify-between gap-4">
        ...
        {viewAllHref && (
          <Link href={viewAllHref} className="shrink-0 text-sm font-medium text-brand-700 hover:text-brand-800">
            View all →
          </Link>
        )}
      </div>
      <ul className="mt-6 grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {items.map((item) => (
          <li key={item.id} className="rounded-xl border border-line bg-surface p-4 shadow-card transition hover:border-line-strong hover:shadow-pop">
            <Link href={hrefFor(item)} className="block focus-visible:outline-none">
