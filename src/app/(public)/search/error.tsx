...
export default function SearchError({ error, reset }: { error: Error & { digest?: string }; reset: () => void }) {
  useEffect(() => {
    reportClientError(error, { boundary: "search" });
  }, [error]);

  return (
    <div className="mx-auto max-w-6xl px-4 py-20 text-center sm:px-6">
      <h1 className="font-display text-2xl text-ink">Search isn&apos;t working right now</h1>
      <p className="mt-2 text-sm text-ink-muted">Please try a different search, or try again in a moment.</p>
